...
router.post("/auth/register", async (req, res) => {
  try {
    ...
    const [user] = await db.insert(usersTable).values({ email, passwordHash, name, phoneNumber }).returning();
    req.session!.userId = user.id;
    const token = signToken(user.id);
    res.status(201).json({
      user: { id: user.id, email: user.email, name: user.name, phoneNumber: user.phoneNumber, createdAt: user.createdAt },
...
router.get("/auth/me", async (req, res) => {
  try {
    ...
    res.json({ id: user.id, email: user.email, name: user.name, phoneNumber: user.phoneNumber, createdAt: user.createdAt });
  } catch (err) {
...
router.patch("/auth/me", async (req, res) => {
  try {
    ...
    req.log.error(err, "Error updating current user");
    res.status(500).json({ error: "Internal server error" });
  }
});
